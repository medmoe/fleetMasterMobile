...
const Driver = () => {
    ...
    const [driverData, setDriverData] = useState<DriverType>(vehicle as DriverType)
    ...
    const submitForm = async () => {
        ...
        try {
            const response = isPostRequest ? await axios.post(`${API}drivers/`, driverData, {headers: {'Content-Type': 'application/json'}, withCredentials: true}) :
                await axios.put(`${API}drivers/${vehicle.id}/`, driverData, {headers: {"Content-Type": "application/json"}, withCredentials: true});
    ...
    const updateResponseData = (response: any, isPostRequest: boolean): void => {
        ...
        if (isPostRequest) {
            ...
            drivers = responseData.drivers?.filter((driver) => driver.id != vehicle.id) || []
