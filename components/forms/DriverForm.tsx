...
const DriverForm = ({handleChange, driverData, handleDateChange, dates, vehicles, submitForm, cancelSubmission}: DriverFormProps) => {
    return (
        <View className={"w-full justify-center items-center"}>
            <View className={"w-[94%] bg-white rounded p-5"}>
                ...
                <View className={"mt-[25px]"}>
                    ...
                    <CustomPicker name={"country"} value={driverData.country || ""} items={countriesLst} handleChange={handleChange}/>
                    <CustomDatePicker date={dates.hire_date} handleChange={handleDateChange} label={"Hire date"} name={"hire_date"}/>
                    <CustomPicker name={"employment_status"} value={driverData.employment_status} items={driverStatuses} handleChange={handleChange}/>
                    {createThirdInputTextPropsBlock(driverData, handleChange).map(({placeholder, value, onChange, name}, idx) => {
                        return <ThemedInputText placeholder={placeholder} value={value} onChange={onChange} name={name} key={idx} containerStyles={"bg-background p-5"}/>
                    })}
                    <CustomPicker name={"vehicle"} value={driverData.vehicle || ""} items={vehicles} handleChange={handleChange}/>
                </View>
